...
function extractCardSeed(destination, providedImageUrl) {
  const url = providedImageUrl ? String(providedImageUrl) : "";
  const fromLocal = url.match(/images\/cards\/pmp-(\d+)\.svg/);
  if (fromLocal) return Number(fromLocal[1]);
  const fromPicsum = url.match(/picsum\.photos\/(?:id|seed\/pmp-)(\d+)/);
  if (fromPicsum) return Number(fromPicsum[1]);
...
function picsumCardUrl(seed) {
  return `https://picsum.photos/seed/pmp-${seed}/1200/800`;
}
...
function getImageUrl(destination, providedImageUrl) {
  ...
  if (
    ...
    !url.includes("images/cards/pmp-")
  ) {
    return url;
  }
  return picsumCardUrl(extractCardSeed(destination, url));
}

function cardImageOnErrorAttr(seed) {
  const local = localCardPath(seed);
  return `if(!this.dataset.fbk){this.dataset.fbk='1';this.src='${local}'}else{this.onerror=null;this.src='${FALLBACK_IMAGE}'}`;
}
...
function renderPickCard(item, { showRemove }) {
  ...
  const seed = extractCardSeed(item.destination, item.imageUrl);
  ...
  return `
    <article class="card">
      <img class="card-image" src="${imageUrl}" alt="${destination}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="${cardImageOnErrorAttr(seed)}" />
      <div class="card-top">
        <h3>${destination}</h3>
      </div>
